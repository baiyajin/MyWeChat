...
可以创建 `pre-commit` 钩子来自动检查：

```bash
# 创建pre-commit钩子
echo @echo off > .git/hooks/pre-commit
echo git ls-files --cached --name-only | findstr /i "\.log" >> .git/hooks/pre-commit
echo if errorlevel 1 exit 0 >> .git/hooks/pre-commit
echo echo 错误：检测到.log文件，请从提交中移除！ >> .git/hooks/pre-commit
...
- `.gitignore` - Git忽略规则
- `清理Git历史中的日志文件.ps1` - PowerShell清理脚本
- `清理Git历史中的日志文件.bat` - BAT清理脚本
